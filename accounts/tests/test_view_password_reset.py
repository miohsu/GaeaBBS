...
class SuccessfulPasswrodResetTests(TestCase):
    def setUp(self):
        email = 'john@163.com'
        User.objects.create_user(username='john', email=email, password='123456abcdef')
        url = reverse('password_reset')
    ...
    def test_send_passwrod_reset_email(self):
...
class InvalidPasswordResetTests(TestCase):
    def setUp(self):
        url = reverse('password_reset')
        self.response = self.client.post(url, {'email': 'donotexist@163.com'})
...
class PasswrodResetDoneTests(TestCase):
    def setUp(self):
        url = reverse('password_reset_done')
        self.response = self.client.get(url)
...
class PasswordResetConfirmTesets(TestCase):
    def setUp(self):
        user = User.objects.create_user(username='john', email='john@163.com', password='123')
        self.uid = urlsafe_base64_encode(force_bytes(user.id)).decode()
